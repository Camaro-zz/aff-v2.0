...
<section class="content">
    <div class="box-header">
        <h3 class="box-title">
            分组查询：
            <select name="token_list" v-on:change="selectToken()" v-model='selectd_token'>
            ...
            时间选项：
            <select name="date_type" v-model="date_type" v-on:change="fetchList()">
            ...
            时区选项：
            <select name="timezone" v-model="timezone" v-on:change="fetchList()">
...
export default {
    ...
    methods: {
        ...
        fetchOffers () {
            this.$http({url: '/camp/' + this.camp_id + '/offer.json?date_type='+this.date_type+'&timezone='+this.timezone, method: 'GET'}).then(function (response) {
        ...
        selectToken(){
            this.$http({url: '/camp/tokens/info/' + this.camp_id + '.json?token_id='+ this.selectd_token+'&date_type='+this.date_type+'&timezone='+this.timezone, method: 'GET'}).then(function (response) {
                if(response.data.status != 'false'){
