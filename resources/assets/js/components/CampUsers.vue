<template>
    <section class="content">
        <h1>设置【{{this_user.name}}】可操作camp</h1>
                    ...
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover">
                            ...
                            <tr v-for="camp in list">
                                <td>
                                    <input type="checkbox" :value="camp.camp_id"  name="camps" v-model="camp_ids" v-if="camp.checked == 1" checked>
                                    <input type="checkbox" :value="camp.camp_id"  name="camps" v-model="camp_ids" v-else>
                                </td>
                                <td>{{camp.camp_id}}</td>
                        ...
                        <div class="box-body pad">
                            <button type="button" @click="saveUserCamp()" class="btn btn-info">保存</button>
                        </div>
                    </div>
                    <!-- /.box-body -->
                </div>
...
<script>
    ...
    export default {
        ...
        ready () {
            this.uid = this.$route.params.hashid
        ...
        methods: {
            fetchCampaigns (page) {
                this.$http({url: '/camp/list.json?page='+page+'&uid=' + this.uid, method: 'GET'}).then(function (response) {
                    this.$set('list', response.data.data)
                    this.$set('all', response.data.count)
                    this.$set('cur', response.data.page)
                    this.$set('this_user', response.data.user);
                })
            },
            ...
            selectAll(){
                var _this = this;
                //实现反选
                if (_this.camp_ids.length === _this.list.length) {
                    _this.camp_ids = [];
                } else {//实现全选
                    _this.camp_ids = [];
                    _this.list.forEach(function(item) {
                        _this.camp_ids.push(item.camp_id);
                    });
                }
            },
            saveUserCamp(){
